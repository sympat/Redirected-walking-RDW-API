...
6. **Resetter**, **Redirector**, **SimulationController**는 각각 Simulation에 사용될 Reset (실제 공간 경계에 부딪혔을 때 어떤 식으로 대응할 지), Redirection (가상 공간 사용자를 어떤 식으로 유도할지), control (사용자가 어떤 방식으로 이동하는지) 의 종류를 결정한다. 
